...
async function fetchApontamentos(empresaIds, from, to, apenasAtivos) {
  ...
  const [rows] = await pool.query(
    ...
    SELECT
      ...
      CASE
        WHEN a.horario IS NOT NULL THEN TIME_FORMAT(a.horario, '%H:%i:%s')
        WHEN UPPER(a.evento)='ENTRADA' AND a.entrada IS NOT NULL THEN TIME_FORMAT(a.entrada, '%H:%i:%s')
        WHEN UPPER(a.evento)='SAIDA'   AND a.saida   IS NOT NULL THEN TIME_FORMAT(a.saida,   '%H:%i:%s')
        ELSE NULL
      END AS horario,
      CASE WHEN UPPER(a.evento)='ENTRADA'
           THEN CASE
                  WHEN a.horario IS NOT NULL THEN TIME_FORMAT(a.horario, '%H:%i')
                  WHEN a.entrada IS NOT NULL THEN TIME_FORMAT(a.entrada, '%H:%i')
                  ELSE NULL
                END
           ELSE NULL
      END AS entrada,
      CASE WHEN UPPER(a.evento)='SAIDA'
           THEN CASE
                  WHEN a.horario IS NOT NULL THEN TIME_FORMAT(a.horario, '%H:%i')
                  WHEN a.saida   IS NOT NULL THEN TIME_FORMAT(a.saida,   '%H:%i')
                  ELSE NULL
                END
           ELSE NULL
      END AS saida,
   ...
   ORDER BY a.data ASC,
            ...
            a.turno_ordem ASC,
            FIELD(UPPER(a.evento),'ENTRADA','SAIDA'),
            COALESCE(a.horario, IF(UPPER(a.evento)='ENTRADA', a.entrada, a.saida)) ASC,
            a.id ASC
    `,
    [empresaIds, from, to]
  );
...
router.get("/dashboard/adm", mustBeAuthed, async (req, res) => {
  try {
    ...
    if (!empresaIds.length) {
      return res.json({ funcionarios: [], escalas: [], apontamentos: [], metricas: null, period: null });
    }
    ...
    return res.json({ funcionarios, escalas, apontamentos, metricas: null, period: { from, to } });
  } catch (e) {
