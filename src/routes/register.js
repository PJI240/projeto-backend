...
async function getOrCreateEmpresaByCNPJ(conn, empresaInput) {
  ...
  // CORREÇÃO: Consulta simplificada e mais eficiente
  const [rows] = await conn.query(
    "SELECT id FROM empresas WHERE cnpj = ? LIMIT 1",
    [cnpjNum] // Já estamos passando o CNPJ sem formatação
  );
  ...
  const [ins] = await conn.query(
    ...
    [
      razao_social,
      nome_fantasia,
      cnpjNum, // CORREÇÃO: Já está sem formatação
      inscricao_estadual,
      data_abertura,
      telefone,
      email,
...
router.post("/vincular-admin", async (req, res) => {
  // CORREÇÃO: Melhor tratamento de autenticação
  let userId = req.userId || null;
  
  // Se não tem userId na requisição, tenta extrair do token
  if (!userId) {
    try {
      ...
      // CORREÇÃO: Verificação mais robusta do token
