...
router.use((req, res, next) =>
  requireAuth(req, res, (err) =>
    err ? res.status(401).json({ ok: false, error: err.message }) : next()
  )
);

/** GET /api/folhas-itens/folhas */
...
/** GET /api/folhas-itens/folhas-funcionarios
 *  Retorna nomes em `funcionario_nome` e, por compatibilidade, nas chaves `funcionarios` e `itens`.
 */
router.get("/folhas-funcionarios", async (req, res) => {
  try {
    ...
    if (!dev) {
      const empresas = await getUserEmpresaIds(req.userId);
      if (!empresas.length) return res.json({ ok: true, funcionarios: [], itens: [] });
    ...
    // Mantém compatibilidade com telas que ainda leem "itens"
    return res.json({ ok: true, funcionarios: rows, itens: rows });
  } catch (e) {
...
/**
 * GET /api/folhas-itens/folhas-itens
...
router.get("/folhas-itens", async (req, res) => {
  try {
    ...
    if (tipo && tipo.toLowerCase() !== "todos") {
      where.push(`UPPER(fi.tipo) = UPPER(?)`); params.push(tipo);
    }
    if (q) {
      where.push(`(
...
/** POST /api/folhas-itens/folhas-itens */
router.post("/folhas-itens", async (req, res) => {
  try {
    const folha_funcionario_id = Number(req.body?.folha_funcionario_id);
    if (!folha_funcionario_id) {
      return res.status(400).json({ ok: false, error: "folha_funcionario_id é obrigatório." });
    }
...
/** PUT /api/folhas-itens/folhas-itens/:id */
...
/** DELETE /api/folhas-itens/folhas-itens/:id */
